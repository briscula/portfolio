...
export default function PortfolioDividendsPage() {
  ...
  useEffect(() => {
    const fetchDividendSummary = async () => {
      ...
      try {
        ...
        const summary = await apiClient.getDividendSummary(
          portfolioId,
          "last12Months",
        );
  ...
  useEffect(() => {
    const fetchHoldings = async () => {
      ...
      try {
        ...
        const response = (await apiClient.getHoldingsYieldComparison(
          portfolioId,
        )) as { holdings: HoldingYieldData[] };
  ...
  if (
    portfoliosError &&
    (portfoliosError.includes("Authentication") ||
      portfoliosError.includes("401"))
  ) {
    redirect("/api/auth/login");
  }
            ...
            {holdingsLoading ? (
              ...
              <div className="text-center py-8">
                ...
                <p className="text-sm text-gray-600">
                  Holdings that paid dividends in the last year will appear
                  here.
                </p>
              </div>
            ) : (
