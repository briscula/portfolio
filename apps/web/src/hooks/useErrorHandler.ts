...
export function useErrorHandler(config: Partial<ErrorHandlerConfig> = {}) {
    ...
    async (errorEntry: ErrorEntry) => {
      ...
      try {
        ...
        if (process.env.NODE_ENV === "development") {
          const errorLog = formatErrorForLogging(
            errorEntry.error,
            errorEntry.context,
          );
  ...
  const handleError = useCallback(
    async (
      ...
      const { maxRetries = 3, autoDismiss = true, report = true } = options;
  ...
  const hasCriticalErrors = activeErrors.some(
