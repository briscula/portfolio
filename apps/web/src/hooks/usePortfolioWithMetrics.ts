...
export function usePortfoliosWithMetrics() {
  ...
  const [portfoliosWithMetrics, setPortfoliosWithMetrics] = useState<
    PortfolioWithMetrics[]
  >([]);
  ...
  const fetchPositionsThrottled = useApiDeduplication((portfolioId: unknown) =>
    portfolioService.getPositions(portfolioId as string, 1, 100),
  );

  // Fetch portfolio metrics with enhanced error handling and offline support
  const fetchPortfolioMetrics = useCallback(
    async (showLoading = true) => {
      if (portfoliosLoading || portfoliosError) {
        setLoading(portfoliosLoading);
        setError(
          portfoliosError ? getUserFriendlyErrorMessage(portfoliosError) : null,
        );
        return;
      }
      ...
      if (
        portfoliosWithMetrics.length > 0 &&
        timeSinceLastRefresh < fiveMinutes &&
        !showLoading
      ) {
        ...
        const portfolioPromises = portfolios.map(async (portfolio) => {
          try {
            ...
            const metrics = {
              ...
              unrealizedGain:
                (summary.totalValueUSD || 0) - (summary.totalAmountUSD || 0),
...
export function useDashboardSummary() {
  ...
  useEffect(() => {
    if (!loading && !error) {
      const calculatedSummary = portfolioService.calculateDashboardSummary(
        portfoliosWithMetrics,
      );
