...
const DEFAULT_ERROR_MESSAGES: Record<string, ErrorMessageConfig> = {
  ...
  'offline': {
    ...
    message: 'It looks like you\'re not connected to the internet. Some features may not work until you reconnect.',
    category: ErrorCategory.NETWORK,
    severity: ErrorSeverity.MEDIUM,
    actionable: true,
    retryable: true,
  ...
  'portfolio_not_found': {
    ...
    message: 'The portfolio you\'re looking for doesn\'t exist or has been deleted.',
  ...
  'portfolio_load_failed': {
    ...
    message: 'We couldn\'t load your portfolio data. This might be a temporary issue.',
    category: ErrorCategory.SERVER,
    severity: ErrorSeverity.MEDIUM,
    actionable: true,
    retryable: true,
...
export function parseError(error: unknown): ErrorMessageConfig {
  ...
  // Portfolio-specific errors
  if (lowerMessage.includes('portfolio') && lowerMessage.includes('not found')) {
