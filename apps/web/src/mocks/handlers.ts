...
export const handlers = [
  ...
  // Position endpoints
  http.get(
    `${API_BASE}/portfolios/:id/positions`,
    async ({ params, request }) => {
      ...
      let filteredPositions = positions.filter(
        (p) => p.portfolioId.toString() === params.id,
      );
  ...
  http.get(
    `${API_BASE}/portfolios/:id/transactions`,
    async ({ params, request }) => {
      await simulateDelay(200);
      ...
      let filteredTransactions = transactions.filter(
        (t) => t.portfolioId === params.id,
      );

      if (sortBy) {
        filteredTransactions = sortData(
  ...
  http.get(`${API_BASE}/portfolios/:id/summary`, async ({ params }) => {
    ...
    const totalDividends = portfolioPositions.reduce(
  ...
  // Dividend endpoints
  http.get(
    `${API_BASE}/portfolios/:id/dividends/summary`,
    async ({ params, request }) => {
      await simulateDelay(150);
  ...
  http.get(
    `${API_BASE}/portfolios/:id/dividends/holdings-yield`,
    async ({ params }) => {
      await simulateDelay(200);
      ...
      const holdings = portfolioPositions.map((pos) => {
        ...
        const yieldOnCost =
          pos.totalCost > 0 ? (totalDividends / pos.totalCost) * 100 : 0;
  ...
  http.get(
    `${API_BASE}/portfolios/:id/dividends/monthly`,
    async ({ params, request }) => {
      await simulateDelay(200);
      ...
      filteredData = filteredData.filter(
        (monthData) => monthData.yearlyData.length > 0,
      );
